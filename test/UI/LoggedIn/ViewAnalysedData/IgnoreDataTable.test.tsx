...
import IgnoreDataTable from '../../../../src/UI/LoggedIn/ViewAnalysedData/Tables/IgnoreDataTable';
...
describe('IgnoreDataTable UI Component', () => {
    describe('General', () => {
        it('Should have a table', () => {
            expect(component.find('table')).toBeTruthy();
        });
        it('should have the correct table column', () => {
            expect(component.find('thead').text()).toBe('Ignored Objects');
        });
        ...
        it('Should show the correct number of rows in the table footer', () => {
            expect(component.find('tfoot').find('p').at(1).text()).toBe('1-5 of 24');
        });
    });
    describe('Table navigation for more than 5 columns', () => {
        ...
        it('Should show the correct number of rows when the option for number of rows has changed', async () => {
            ...
            expect(component.find('tfoot').find('p').at(1).text()).toBe('1-10 of 24');
            await setNumberOfRows(25);
            expect(component.find('tfoot').find('p').at(1).text()).toBe('1-24 of 24');
            await setNumberOfRows(-1);
            expect(component.find('tfoot').find('p').at(1).text()).toBe('1--1 of 24');
        ...
        it('Should go to the last page when the last page button is clicked', async () => {
            await clickLastPage();
            const element = component.find('tbody').find('tr').at(0);
            expect(element.find('td').text()).toEqual('"Object 20":{...}2 items');
        });
